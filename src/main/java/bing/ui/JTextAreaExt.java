...
public class JTextAreaExt extends JTextArea implements MouseListener {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JTextAreaExt.class);
    private JPopupMenu popMenu = null; // 弹出菜单
    private JMenuItem menuItemFmt = null, menuItemCopy = null, menuItemCut = null, menuItemClear = null; // 功能菜单
    private static final ImageIcon ICON_COPY = new ImageIcon(JTextAreaExt.class.getResource(Constants.ICON_COPY_PATH));
    private static final ImageIcon ICON_CUT = new ImageIcon(JTextAreaExt.class.getResource(Constants.ICON_CUT_PATH));
    private static final ImageIcon ICON_CLEAR = new ImageIcon(JTextAreaExt.class.getResource(Constants.ICON_CLEAR_PATH));
    private static final ImageIcon ICON_FMT = new ImageIcon(JTextAreaExt.class.getResource(Constants.ICON_FMT_PATH));
    ...
    private void init() {
        ...
        menuItemFmt = new JMenuItem(Constants.TEXT_FMT);
        menuItemFmt.setIcon(ICON_FMT);
        ...
        menuItemFmt.addActionListener(new ActionListener() {
            @Override
    ...
    public void action(ActionEvent e) {
        ...
        if (cmd.equals(menuItemFmt.getText())) { // 格式化
            formatJson();
        } else if (cmd.equals(menuItemCopy.getText())) { // 复制
            this.copy();
    ...
    private void formatJson() {
        ...
        if (JsonUtils.valid(text)) {
            try {
                String formatJson = JsonUtils.formatJson(text);
                this.setText(formatJson);
            } catch (OperationException e) {
                LOGGER.error(e.getMessage());
            } catch (Exception e) {
                LOGGER.error(ExceptionUtils.createExceptionString(e));
                LOGGER.error("JSON格式化失败，详情查看日志");
            }
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON3) {
            menuItemFmt.setEnabled(allowFmt());
